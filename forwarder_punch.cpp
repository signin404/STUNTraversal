...
void ShowToastNotification(const std::wstring& message) {
    ...
    try {
        ...
        // 增强版XML: 添加了launch属性，这有时是必需的
        std::wstring xml_content = L"<toast launch='action=view'><visual><binding template='ToastGeneric'><text>NAT Forwarder: Public Endpoint Changed</text><text>";
        xml_content += message;
        ...
        // --- 核心修改在这里 ---
        // 将错误信息写入日志文件
        std::wofstream log_file("toast_log.txt", std::ios_base::app);
        if (log_file.is_open()) {
            // 设置locale以正确处理宽字符写入UTF-8文件
            log_file.imbue(std::locale(log_file.getloc(), new std::codecvt_utf8<wchar_t>));
            log_file << L"Toast notification failed!" << std::endl;
            log_file << L"HRESULT: 0x" << std::hex << e.code() << std::endl;
            log_file << L"Message: " << e.message().c_str() << std::endl;
            log_file << L"---------------------------------" << std::endl;
            log_file.close();
        }
    }
