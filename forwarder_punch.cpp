...
struct Config {
    ...
    int local_punch_port; // 用于打洞和监听的本地端口
    std::string forward_host;
    int forward_port;
    std::string priming_host; // 用于预热NAT的公网主机
    int keep_alive_ms;
...
Config ReadIniConfig(const std::string& filePath) {
    ...
    config.retry_interval_ms = GetPrivateProfileIntA("TCP_HolePunch", "RetryIntervalMS", 30000, filePath.c_str()); // 延长重试间隔
...
void PortForwardingThread(Config config) {
    do {
        ...
        if (!GetPublicEndpoint_TCP(config, public_ip, public_port)) {
            SetColor(RED);
            std::cerr << "[失败] STUN 请求失败。请检查网络或 STUN 服务器。" << std::endl;
            if (config.auto_retry) { std::this_thread::sleep_for(std::chrono::milliseconds(config.retry_interval_ms)); continue; } 
            else break;
        }
        ...
        if (bind(listener_sock, (sockaddr*)&local_addr, sizeof(local_addr)) == SOCKET_ERROR) {
            SetColor(RED);
            std::cerr << "[错误] 无法绑定本地端口 " << config.local_punch_port << "。可能已被占用。" << std::endl;
        ...
        // 步骤 3: 使用同一个套接字打洞并立即切换到监听模式
        SetColor(CYAN);
        std::cout << "[步骤 3] 正在通过连接 '" << config.priming_host << "' 来为公网端口打洞..." << std::endl;
        addrinfo* prime_res = nullptr;
        getaddrinfo(config.priming_host.c_str(), "80", nullptr, &prime_res);
        if (prime_res) {
            // 非阻塞 connect 只为发送SYN包
            u_long mode = 1;
            ...
            std::this_thread::sleep_for(std::chrono::milliseconds(200)); // 等待SYN包发出
            ...
            ioctlsocket(listener_sock, FIONBIO, &mode);
            SetColor(GREEN);
            std::cout << "[成功] 打洞完成！现在公网端口 " << public_port << " 已开启并监听。" << std::endl;
            SetColor(YELLOW);
            ...
            while (true) {
                ...
                if (peer_sock == INVALID_SOCKET) {
                    SetColor(RED);
                    std::cerr << "[警告] accept() 失败。可能NAT映射已过期，准备重试..." << std::endl;
                    break; // 跳出内层循环，触发外层重试逻辑
                }
                // 为每个新连接创建一个独立的线程来处理转发
                std::thread(HandleNewConnection, peer_sock, config).detach();
            }
        } else {
             SetColor(RED);
             std::cerr << "[错误] 无法解析预热主机 '" << config.priming_host << "'。" << std::endl;
        }
        closesocket(listener_sock);
        if (config.auto_retry) {
             SetColor(YELLOW);
             std::cout << "[准备重试] 等待 " << config.retry_interval_ms / 1000 << " 秒后将自动重试..." << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    SetColor(YELLOW);
    std::cout << "--- TCP NAT 端口转发器 ---" << std::endl;
